...
│   │   │   ├── logs-events.js     # Logs & événements frontend
│   │   │   └── faq.js             # FAQ intégrée
...
│   │   └── tests/                 # Frontend tests (functional)
│   │       ├── faq-structure.test.mjs
│   │       ├── html-css-structure.test.mjs
│   │       ├── logs-events.test.mjs
│   │       ├── marex-permission.test.mjs
│   │       ├── marexcode-structure.test.mjs
│   │       └── tool-loop-guard.test.mjs
│   ├── tests/                     # Frontend tests (structure & routing)
│   │   ├── router.test.mjs
...
│   ├── websearch.py               # WebSearch backend : 6 providers en cascade (Tavily → Exa → Brave → Jina → SearXNG → DDG)
...
```
variables.css + layout.css + chat.css + marexcode.css + logs-events.css + components.css + canvas.css + catalog.css + storage.css + menu.css
→ minifié → style.css
```
...
| `logs-events.css` | Logs & événements frontend |
...
```bash
# Tests structure & routing (static/tests/)
...
node --test static/tests/canvas-structure.test.mjs # Canvas factory
node --test static/tests/chat-structure.test.mjs   # Chat factory (note: échoue en Node.js — window requis)
...
# Tests fonctionnels (static/js/tests/)
node --test static/js/tests/faq-structure.test.mjs          # Structure données FAQ (9 tests)
node --test static/js/tests/html-css-structure.test.mjs     # HTML div-balance + CSS brace-balance (29 tests)
node --test static/js/tests/logs-events.test.mjs            # Logs & Events (6 tests)
node --test static/js/tests/marex-permission.test.mjs       # Permissions granulaires (4 tests)
node --test static/js/tests/marexcode-structure.test.mjs    # Structure page /marexcode/
node --test static/js/tests/tool-loop-guard.test.mjs        # Garde-fou boucle tools (7 tests)
