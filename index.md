...
│   │   │   ├── tool-search.js     # Recherche web + outils Marexcode (Ls/Read/Write/Edit/Grep/Bash/TodoWrite) + streamModelWithTools
...
│   ├── marexcode.py               # Backend Marexcode : sandbox exec + sessions + tree (mixin MarexcodeMixin)
...
**Body** : `{"tool": "Ls|Bash|Read|Write|Edit|Grep", "args": {...}}` — `TodoWrite` est **virtuel côté client** (CustomEvent `marexcode-todo`, pas de `/api/exec`).
