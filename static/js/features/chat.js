...
export function createChat(deps) {
    ...
    function createStreamRenderer(e, t, n) {
        ...
        return {
            ...
            flush() { f && (cancelAnimationFrame(f), f = null), ts = null, hideWait(); _stopSpinner(); o += a, a = ""; const e = t(); e && (e.innerHTML = marked.parse(o), addCodeCopyButtonsFn(e)); }
        };
    }
    ...
    function addMessage(e, t, n, o, a, r, s) {
        ...
        if (i.className = `message message-${e}`, "assistant" === e && a) {
            ...
            const b = document.createElement("div"); b.className = "thinking-content", b.innerHTML = marked.parse(a), I.appendChild(b), i.appendChild(I);
        }
        if (Array.isArray(t)) {
        ...
        if ("assistant" === e && c) l.innerHTML = marked.parse(c), addCodeCopyButtonsFn(l); else if ("assistant" !== e || c) l.textContent = c; else if ("string" == typeof t) { const R = document.createElement("span"); R.className = "generation-placeholder", R.textContent = "Génération en cours...", l.appendChild(R); }
    ...
    async function regenerateLastResponse() {
        ...
        if (STATE.currentImageModel && !getImageModelEditeur(STATE.currentImageModel) && (STATE.currentModel = STATE.currentImageModel, STATE.currentImageModel = null), STATE.currentImageModel) {
            ...
            generateImage(STATE.currentImageModel, e, (e => { if (!STATE._activeStreams.has(t)) return; STATE._activeStreams.delete(t); const r = isStreamActive(t, n); if (!(e.images && 0 !== e.images.length || e.text)) return void (t === STATE.conversationId && (o && o.parentNode && o.parentNode.removeChild(o), STATE.isStreaming = false, STATE.currentAbortController = null, updateSendButton(), addRegenBtn())); if (r) { endStreaming(o), e.images.length > 0 && o.insertBefore(buildImagesContainer(e.images, { altText: "Image générée" }), o.firstChild); const t = o.querySelector(".message-text"); t && e.text ? (t.innerHTML = marked.parse(e.text), addCodeCopyButtonsFn(t)) : t && t.remove(); } const c = imageResultToContent(e), d = (Date.now() - a) / 1e3, u = e.usage?.output_tokens || 0; n.push({ role: "assistant", content: 1 === c.length && "text" === c[0].type ? c[0].text : c, generationTime: d, outputTokens: u, model: l }); const p = getImageTarif(l), m = _resolveImageCost(p, e.usage, e.imageCount, s, i); r ? (attachCanvasBeforeToLastAssistant(), setGenTimeOnLastAssistant(d, u, l), e.usage && (STATE.totalInputTokens += e.usage.input_tokens || 0, STATE.totalOutputTokens += e.usage.output_tokens || 0), STATE.totalCost += m.tokenCost, STATE.totalImageCost += m.imageCost, addCostForModel(l, e.usage?.input_tokens || 0, e.usage?.output_tokens || 0, m.total), updateTokenDisplay(), saveConversation(), scrollToBottom(true), addRegenBtn(), STATE.isStreaming = false, STATE.currentAbortController = null, updateSendButton(), promptInput.focus()) : _saveConvById(t, n, { tokensIn: e.usage?.input_tokens || 0, tokensOut: e.usage?.output_tokens || 0, cost: m.tokenCost, imageCost: m.imageCost, modelKey: l }); }), (e => { if (STATE._activeStreams.delete(t), t !== STATE.conversationId) return n.push({ role: "assistant", content: STREAM_ERROR_CONTENT, error: true }), void _saveConvById(t, n, { modelKey: l }); const a = n[n.length - 1]; handleApiError(o, getTextFromContent(a?.content), [], l || "", e); }), r, STATE.currentAbortController.signal, document.getElementById("image-format-select").value, getImageParams());
        } else {
            let e = "", r = "", s = "";
