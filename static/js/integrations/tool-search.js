...
var MAREXCODE_TOOLS = [
  {type:"function",function:{name:"Bash",description:"Run a shell command (whitelist: ls,cat,grep,git,node,python3,npm,npx,head,tail,wc,find,sed,awk,echo,mkdir,touch,rm,cp,mv,pwd,date). Timeout 10s default, 60s max.",parameters:{type:"object",properties:{command:{type:"string",description:"Shell command"},timeout:{type:"integer",description:"Seconds (default 10, max 60)"}},required:["command"]}}},
  {type:"function",function:{name:"Read",description:"Read a file, with offset/limit pagination.",parameters:{type:"object",properties:{file_path:{type:"string",description:"Relative path"},offset:{type:"integer",description:"Start line, 1-based"},limit:{type:"integer",description:"Max lines"}},required:["file_path"]}}},
  {type:"function",function:{name:"Write",description:"Write or overwrite a file.",parameters:{type:"object",properties:{file_path:{type:"string"},content:{type:"string"}},required:["file_path","content"]}}},
  {type:"function",function:{name:"Edit",description:"Replace one occurrence of text, returns unified diff.",parameters:{type:"object",properties:{file_path:{type:"string"},old:{type:"string",description:"Exact text to replace"},new:{type:"string",description:"Replacement text"}},required:["file_path","old","new"]}}},
  {type:"function",function:{name:"Grep",description:"Search for a pattern in project files.",parameters:{type:"object",properties:{pattern:{type:"string"},path:{type:"string",description:"Default: project root"},limit:{type:"integer"}},required:["pattern"]}}},
  {type:"function",function:{name:"Glob",description:"Find files by pattern (e.g. **/*.ts).",parameters:{type:"object",properties:{pattern:{type:"string"}},required:["pattern"]}}},
  {type:"function",function:{name:"Ls",description:"List the full workspace file tree. Use this first.",parameters:{type:"object",properties:{},required:[]}}},
  {type:"function",function:{name:"TodoWrite",description:"Update the task list (multi-step plan).",parameters:{type:"object",properties:{todos:{type:"array",items:{type:"object",properties:{content:{type:"string"},status:{type:"string",enum:["pending","in_progress","completed"]}}},description:"Tasks with statuses"}},required:["todos"]}}},
  {type:"function",function:{name:"LSP",description:"Code intelligence via LSP: definition, references, hover, symbols. Requires an installed LSP server.",parameters:{type:"object",properties:{operation:{type:"string",enum:["definition","references","hover","symbol"]},file:{type:"string"},line:{type:"integer",description:"0-based"},character:{type:"integer",description:"0-based"}},required:["operation","file","line","character"]}}},
  {type:"function",function:{name:"RunScript",description:"Run a python/node script in sandbox (prefer over Bash for code). Timeout 30s default, 60s max.",parameters:{type:"object",properties:{language:{type:"string",enum:["python","node"]},code:{type:"string"},timeout:{type:"integer"}},required:["language","code"]}}},
];
