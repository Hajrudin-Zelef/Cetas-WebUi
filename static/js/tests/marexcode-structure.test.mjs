...
test('chat.js : badge Thinking marqué done dès le démarrage d’un outil', () => {
    ...
    assert.match(helper, /_markAllThinkDone\(\)/, 'markThinkingDone doit déléguer à _markAllThinkDone');
    ...
    const sweep = src.slice(src.indexOf('function _markAllThinkDone()'), src.indexOf('function markThinkingDone()'));
    assert.match(sweep, /classList\.add\('done'\)/, 'tous les badges doivent passer en done');
    assert.match(sweep, /thinkShimmer\.stop\(\)/, 'le shimmer doit être arrêté');
    assert.match(sweep, /classList\.remove\('text-shimmer'\)/, 'le shimmer des badges précédents doit être retiré');
});
