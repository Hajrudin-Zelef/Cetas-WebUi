...
function setBlock(container, index, html, highlightable) {
    ...
    // Jamais de highlight async sur le bloc "live" en cours de stream : le code
    // grossit à chaque frame et une surlignage retardé écraserait le contenu.
    if (highlightable) highlight(el);
}
...
function render(container, text, streaming) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        setBlock(container, i, parseMarkdown(blocks[i]), true);
    }
    let total = blocks.length;
    if (streaming || tail.trim()) {
        setBlock(container, total, parseMarkdown(tail), !streaming);
        total += 1;
    }
