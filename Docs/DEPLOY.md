...
> **Stack :** Debian 13 + Docker + Nginx + SearXNG
...
```bash
# Fichiers requis
scp votre-chemin/scripts/setup.py                root@VPS:/opt/cetas/scripts/
...
- `scripts/setup.py` — configuration initiale vault
...
services:
  cetas:
    ...
    volumes:
      ...
      - ./.opencode/skills:/usr/share/nginx/html/.opencode/skills:ro
  ...
  searxng:
    ...
    env_file:
      - .env.docker
    environment:
      - SEARXNG_BASE_URL=http://localhost:8904/
    volumes:
...
CETAS_CORS_ORIGINS=https://votre-domaine.com,http://localhost:8901
...
1. **server/server.py** → Ajouter dans `PROVIDER_CONFIG` :
```python
"monprovider": {
...
2. **server/server.py** → Ajouter dans `PROXY_ALLOWED_PATHS` :
```python
"monprovider": ["/v1/chat/completions"],
```

3. **static/js/data/models.js** → Ajouter les modèles dans `MODELS_DATA` :
```javascript
{ id: "mon-modele", label: "Mon Modèle", editeur: "monprovider", inputPer1M: 1, outputPer1M: 5 }
```

4. **static/partials/head.html** → Si domaine unique, ajouter au CSP `connect-src`

5. **static/images/** → Ajouter l'icône SVG du provider

6. **.env** → Chiffrer la clé API avec `scripts/setup.py`
...
| `CETAS_LLAMACPP_URL` | — | URL LLaMA.cpp (optionnel) |
| `CETAS_OLLAMA_URL` | — | URL Ollama (optionnel) |
| `CETAS_LMSTUDIO_URL` | — | URL LM Studio (optionnel) |
...
| 502 Bad Gateway | Container pas sur port 8901 — vérifier docker-compose.yml |
...
| Recherche web échoue | Vérifier `websearch.py` et les clés API des providers (Tavily, Exa, Brave) |
